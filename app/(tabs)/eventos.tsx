...
import { updateUserPushToken, updateNotificationPrefs } from "../../services/UserService";
...
// Registra el token de Expo y lo guarda en backend si es nuevo/diferente
useEffect(() => {
  (async () => {
    try {
      // 1) Determinar UID actual
      ...
      // 3) Guardarlo en backend (idempotente si no cambia)
      await updateUserPushToken(uid, token);
    } catch (e) {
      // silencioso o log si quieres
      console.log("No se pudo registrar el push token:", e);
    }
  })();
}, []);
